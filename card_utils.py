...
def hole_card_draw(hole_card,community_card):
    community_card = _fill_community_card(community_card, used_card=hole_card+community_card)
    return (community_card,HandEvaluator.gen_hand_rank_info(hole_card,community_card))

def estimate_hole_card_draw_rate(nb_simulation,hole_card,community_card):
    def add_to_scenarios(n,s):
        sc = s
        for x in range(n):
            sc[x]+=1
        return sc

    strengths = ["HIGHCARD","ONEPAIR","TWOPAIR","THREECARD", "STRAIGHT", "FLASH","FULLHOUSE", "FOURCARD","STRAIGHTFLASH"]
    scenarios = [0 for x in range(9)]
    info = ""
    for x in range(nb_simulation):
        tested_community_card,hand_info = hole_card_draw(hole_card,community_card)
        if hand_info["hand"]["strength"] == "ONEPAIR":
            cc = [card.rank for card in tested_community_card] + [0]
            highest_card = max(cc)
            if hand_info["hand"]["high"] >= highest_card:
                scenarios = add_to_scenarios(1,scenarios)
            else:
                scenarios = add_to_scenarios(0,scenarios)
            
        for index, strength in enumerate(strengths):
            if hand_info["hand"]["strength"] == strength:
                scenarios = add_to_scenarios(index,scenarios)
    scenarios = [sc/nb_simulation for sc in scenarios]
    return scenarios
