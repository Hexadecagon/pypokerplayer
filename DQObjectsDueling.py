...
class DQN(object): #Implements Double DQN, with the slow-moving copy of the network instead of two separately trained ones. This is fine.
    def __init__(self, dim, u0,u, name):
        ...
        for index,unit in enumerate(u0):
            if index == 0:
        ...
        for index,unit in enumerate(u):
            if index == 0:
                self.LV.append(tf.layers.dense(inputs = self.L[len(self.L)-1], units = unit, activation = tf.nn.relu, name = "LV"+str(index)+str(name)))
                self.LA.append(tf.layers.dense(inputs = self.L[len(self.L)-1], units = unit, activation = tf.nn.relu, name = "LA"+str(index)+str(name)))
                self.TV.append(tf.layers.dense(inputs = self.T[len(self.T)-1], units = unit, activation = tf.nn.relu, name = "TV"+str(index)+str(name)))
                self.TA.append(tf.layers.dense(inputs = self.T[len(self.T)-1], units = unit, activation = tf.nn.relu, name = "TA"+str(index)+str(name)))       
            else:
...
#        print( self.sess.run(self.advantage_function, feed_dict={self.x:[features]}).tolist()[0])
    ...
    def update_target_network(self): #IT WORKS, I THINK.
        ...
        old_weights  = [self.get_weights_and_biases(L) for L in LVs+ LAs +Ls+ ["Value"+str(self.name),"Advantage"+str(self.name)]]
        
        names = ["TV" + str(n) + str(self.name) for n in range(len(self.TV))] + ["TA" + str(n) + str(self.name) for n in range(len(self.TA))]+ ["T" + str(n) + str(self.name) for n in range(len(self.T))] +["TValue"+str(self.name),"TAdvantage"+str(self.name)]
